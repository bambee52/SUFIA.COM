...
<?php
  if($_POST['price'] =='상관무'){
    ...
    echo '저렴한 가격 원행!!';
    mysqli_query($con, "delete from a where price>20000");
  //  mysqli_close($con);
              ...
              <header id="header">
                <a href="index.php" class="a"><strong>수피아</strong>.com</a>
							...
							<!-- Content -->
								<section>
									<header class="main">
                                    <p style="font-size:2.5em;"><strong> 아래의 식물들을 추천합니다. </strong> </p>
                                    </header>
                                    
                    <form method="post" action= "find_plants_1.php">
                        <?php
                        mysqli_query($con, "delete from a; insert into a select * from copy;"); 
                       
                       ?>
                        <p> <input type="submit" value="repeat"> </p>
                      ...
                      <?php
                        $result = mysqli_query($con, "select title, img, price from a");

                          while($row = $result->fetch_array()){
                        //  while($row=mysqli_fetch_array($result)){
                            $like= '<form action="like_ok.php" method="POST">
                                    <input type="hidden" name="title" value="'.$row['title'].'">
                                    <input type="hidden" name="img" value="'.$row['img'].'">
                                    <input type="image" src="jjim.png" width=90 height=30>

                                    </form>
                                    ';
                                   ?>

                              <table style="width:30%; float:left; text-align:center;">
                                <tr height="500">
                                  <td > <?php
                                    $src = $row['img'];
                                    echo "<img src='$src' width='250px' /> &emsp; <br>";
                                    ?></td>
                                </tr>
